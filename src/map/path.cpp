 ...
 *------------------------------------------*/
bool path_search_long(struct shootpath_data *spd,int16 m,int16 x0,int16 y0,int16 x1,int16 y1,cell_chk cell, int32 maxdist)
{
	int32 dx, dy;
	int32 wx = 0, wy = 0;
	int32 weight;
 ...
 *------------------------------------------*/
bool path_search(struct walkpath_data *wpd, int16 m, int16 x0, int16 y0, int16 x1, int16 y1, int32 flag, cell_chk cell, int32 maxdist)
{
	...
	if (flag&2)
		return path_search_long(nullptr, m, x0, y0, x1, y1, cell, maxdist);
	...
	if (flag&1) {
		...
		for(;;) {
			...
			int32 usedlength = abs(x - x0);
			if (usedlength < abs(y - y0)) usedlength = abs(y - y0);

			int32 needlength = abs(x - x1);
