...
int32 status_calc_pc_sub(map_session_data* sd, uint8 opt)
{
	...
	npc_script_event( *sd, NPCE_REFINESETEVENT );
...
int32 status_change_end(struct block_list* bl, enum sc_type type, int32 tid)
{
	...
	switch(type) {
		case SC_AUTOATTACK:
			...
			if(tid != INVALID_TIMER){
				clif_displaymessage(sd->fd, "Auto-Battle Disable - ( The device's battery is depleted. )");
				pc_setaccountreg(sd, add_str("#BotGameTimer"), 0);
			}
			break;
		case SC_KEEPING:
		case SC_BARRIER: {
		...
		if (--(sce->val4) > 0) {
			...
			{//Attack skills
				if (sd->aa.autoslavemode == 0) {
					if (!skip && sd->aa.target_id > 0 && !sd->aa.itempick_id && target != nullptr){//Attack
						...
						} else if(!sd->aa.teleport.use_teleport || !sd->aa.teleport.use_flywing || !sd->aa.teleport.use_l1flywing || !sd->aa.teleport.use_gflywing || !sd->aa.teleport.use_nflywing || !sd->aa.teleport.use_iflywing || !sd->aa.teleport.use_n2flywing || !sd->aa.teleport.use_cflywing || !sd->aa.teleport.use_c2flywing){
								...
								for(auto &itAutoattackskills : skills_temp_list){
									...
									if(last_tick >= sd->aa.skill_cd 
										...
										if (skill_get_inf(itAutoattackskills.skill_id) & INF_ATTACK_SKILL || skill_get_inf(itAutoattackskills.skill_id) & INF_GROUND_SKILL) {
											int32 aa_skill_range = skill_get_range(itAutoattackskills.skill_id, itAutoattackskills.skill_lv);
											if (aa_skill_range < 0)
											...
											if (!check_distance_bl(&sd->bl, target, aa_skill_range) || !path_search_long(nullptr, sd->bl.m, sd->bl.x, sd->bl.y, target->x, target->y, CELL_CHKWALL)) {
			...
			if(!sd->aa.itempick_id && sd->aa.target_id)
				...
				//ShowError("MOVE - char %d [%s] - sce->val4 %d - skip %d, pc_issit %d, sd->aa.target_id %d sd->aa.itempick_id %d, attack_ %d flywing %d \n",sd->status.char_id,sd->status.name,sce->val4,skip,pc_issit(sd),sd->aa.target_id,sd->aa.itempick_id,attack_,flywing_);
					if (sd->aa.manualwalk == 0) {
						if (sd->aa.autoslavemode == 0) {
							if(!skip && !pc_issit(sd) && ((sd->aa.target_id == 0 && sd->aa.itempick_id == 0) || attack_ > 10000 || idle_ > 15) && flywing_ == false){
								const int32 d=battle_config.feature_autoattack_move_max; // max number of cell a player can move
								const int32 aa_min_move=battle_config.feature_autoattack_move_min; // min number of cell a player can move
								int32 k, r, rdir, dx, dy, max, tx, ty;
...
int32 buildin_autopick_sub(struct block_list *bl, va_list ap){
	int32 *itempick_id = va_arg(ap, int32 *);
	int32 src_id = va_arg(ap, int);
...
bool aa_check_item_pickup(map_session_data *sd, struct block_list *bl)
{
	...
	if(bl && bl->type == BL_ITEM && bl->m == sd->bl.m && !pc_cant_act(sd)){
		...
		if(sd->aa.pickup_item_config == 1 && !sd->aa.pickup_item_id.empty()){
			for (int32 i=0; i<sd->aa.pickup_item_id.size(); i++){
		...
		if (path_search(nullptr, sd->bl.m, sd->bl.x, sd->bl.y, bl->x, bl->y, 1, CELL_CHKNOREACH) && distance_xy(sd->bl.x, sd->bl.y, bl->x, bl->y) < 11){
			//ShowError("E - Path OK \n");
			return true;
		}
	}
...
uint32 aa_check_item_pickup_onfloor(map_session_data *sd){
...
bool aa_check_target(map_session_data *sd, uint32 id)
{
	struct block_list *bl = map_id2bl(id);

	if (bl && path_search(nullptr, sd->bl.m, sd->bl.x, sd->bl.y, bl->x, bl->y, 0, CELL_CHKNOPASS) && distance_xy(sd->bl.x, sd->bl.y, bl->x, bl->y) < MAX_WALKPATH){
		...
		if (md && md->status.hp > 0
			...
			if(!sd->aa.target_id && sd->aa.mobs.id.size() > 0){
				for(int32 j_ = 0; j_ < sd->aa.mobs.id.size(); j_++){
...
int32 buildin_autoattack_sub(struct block_list *bl, va_list ap)
{
	int32 *target_id = va_arg(ap, int32 *);
	int32 src_id = va_arg(ap, int);
	struct block_list *src = map_id2bl(src_id);
	map_session_data *sd = map_id2sd(src->id);
	if (!src || !bl)
...
uint32 aa_check_target_alive(map_session_data *sd){
	
	if (!aa_check_target(sd, sd->aa.target_id)){
		int32 i_, target_id_ = 0;
...
bool aa_teleport(map_session_data *sd){
	...
	if(!sd->aa.teleport.use_l1flywing && flywing_ == false){
		i_ = pc_search_inventory(sd, 12003);
		if (i_ >= 0){
			pc_useitem(sd, i_);
			flywing_ = true;
		}
	}
	if(!sd->aa.teleport.use_gflywing && flywing_ == false){
		i_ = pc_search_inventory(sd, 12212);
		if (i_ >= 0){
			pc_useitem(sd, i_);
			flywing_ = true;
		}
	}
	if(!sd->aa.teleport.use_nflywing && flywing_ == false){
	...
	if(!sd->aa.teleport.use_iflywing && flywing_ == false){
	...
	if(!sd->aa.teleport.use_n2flywing && flywing_ == false){
		i_ = pc_search_inventory(sd, 23280);
		if (i_ >= 0){
			pc_useitem(sd, i_);
			flywing_ = true;
		}
	}
	if(!sd->aa.teleport.use_cflywing && flywing_ == false){
		i_ = pc_search_inventory(sd, 23288);
		if (i_ >= 0){
			pc_useitem(sd, i_);
			flywing_ = true;
		}
	}
	if(!sd->aa.teleport.use_c2flywing && flywing_ == false){
		i_ = pc_search_inventory(sd, 23338);
		if (i_ >= 0){
			pc_useitem(sd, i_);
			flywing_ = true;
		}
...
int32 aa_arrowchange(map_session_data * sd, struct mob_data *md){
	int32 arrowelement;
	...
	if (best > -1) {
		...
		clif_displaymessage(sd->fd, "Auto-Battle Disable - ( There is no arrows left to use! )");
		status_change_end(&sd->bl, SC_AUTOATTACK);
		return 0;
	}
...
int32 aa_bulletchange(map_session_data * sd, mob_data *md){
	unsigned short arrows[] = {
	...
	int32 i, j;
	int32 best = -1; int32 bestprio = -1;
	bool eqp = false; int32 bestelem = -1;
...
int32 aa_kunaichange(map_session_data * sd, struct mob_data *md, int32 rqamount){
	unsigned short arrows[] = {
...
int32 aa_cannonballchange(map_session_data * sd, struct mob_data *md){
	unsigned short arrows[] = {
...
bool aa_elemstrong(struct mob_data *md, int32 ele){
	if(!md || &md->bl == nullptr)
		return 0;
...
bool aa_elemallowed(struct mob_data *md, int32 ele){
	if(!md || &md->bl == nullptr)
		return 1;
