...
struct Script_Config script_config = {
	...
	"OnPCRefineSetEvent", //refine_set_event_name
	// NPC related
...
BUILDIN_FUNC(getautoattackstring)
{
	...
	switch(id){
			...
			switch(index){
				case 0:
					if(sd->aa.autositregen.is_active)
						buf += std::string("[^00FF00Activate^000000] - Sit to Rest");
					else
						buf += std::string("[^FF0000Deactivate^000000] - Sit to Rest");

					break;
				case 1:
					if(sd->aa.autositregen.min_hp > 0)
						buf += "Sit ^32CD32HP^000000 below ^D2691E" + std::to_string(sd->aa.autositregen.min_hp) + "%^000000\nStand ^32CD32HP^000000 above ^D2691E" + std::to_string(sd->aa.autositregen.max_hp) + "%^000000\n";
					break;
				case 2:
					if(sd->aa.autositregen.min_sp)
						buf += "Sit ^0000FFSP^000000 below ^D2691E" + std::to_string(sd->aa.autositregen.min_sp) + "%^000000\nStand ^0000FFSP^000000 above ^D2691E" + std::to_string(sd->aa.autositregen.max_sp) + "%^000000\n";
					break;
			}
			...
			if(index > 0){
				...
				if(item_data->type == IT_USABLE){ // check if item is type usable
					...
					if(itAutobuffitem != sd->aa.autobuffitems.end()){
						...
						buf += std::string("[^00FF00ON^000000] - ^FF0000(") + std::to_string(index) + ")^000000 " + item_data->ename.c_str() + " : " + std::to_string(autobuffitem.delay) + "s";
					} else {
						buf += std::string("[^FF0000OFF^000000] - ^FF0000(") + std::to_string(index) + ")^000000 " + item_data->ename.c_str() + "";					
					}
				}
			}
		...
		case 6:	// Melee attack
			if(index == 0){
				if(!sd->aa.stopmelee)
					buf += std::string("[^00FF00Activate^000000] - Melee attack");
				else
					buf += std::string("[^FF0000Deactivate^000000] - Melee attack");
			}
			script_pushstrcopy(st, buf.c_str());
			break;
		...
		case 7:	// Teleport
			if(index == 0){
				if(!sd->aa.teleport.use_teleport)
					buf += std::string("[^00FF00Activate^000000] - Teleport Skill");
				else
					buf += std::string("[^FF0000Deactivate^000000] - Teleport Skill");
			}
			if(index == 1){
				if (pc_search_inventory(sd, 601) >= 0)
					if(!sd->aa.teleport.use_flywing)
						buf += std::string("[^00FF00Activate^000000] - Fly Wing");
					else
						buf += std::string("[^FF0000Deactivate^000000] - Fly Wing");
			}
			if(index == 2){
				if (pc_search_inventory(sd, 12003) >= 0)
					if(!sd->aa.teleport.use_l1flywing)
						buf += std::string("[^00FF00Activate^000000] - Level 1 Teleport");
					else
						buf += std::string("[^FF0000Deactivate^000000] - Level 1 Teleport");
			}
			if(index == 3){
				if (pc_search_inventory(sd, 12212) >= 0)
					if(!sd->aa.teleport.use_gflywing)
						buf += std::string("[^00FF00Activate^000000] - Giant Fly Wing");
					else
						buf += std::string("[^FF0000Deactivate^000000] - Giant Fly Wing");
			}
			if(index == 4){
				if (pc_search_inventory(sd, 12323) >= 0)
					if(!sd->aa.teleport.use_nflywing)
						buf += std::string("[^00FF00Activate^000000] - Novice Fly Wing");
					else
						buf += std::string("[^FF0000Deactivate^000000] - Novice Fly Wing");
			}
			if(index == 5){
				if (pc_search_inventory(sd, 12887) >= 0)
					if(!sd->aa.teleport.use_iflywing)
						buf += std::string("[^00FF00Activate^000000] - Infinite Flywing");
					else
						buf += std::string("[^FF0000Deactivate^000000] - Infinite Flywing");
			}
			if(index == 6){
				if (pc_search_inventory(sd, 23280) >= 0)
					if(!sd->aa.teleport.use_n2flywing)
						buf += std::string("[^00FF00Activate^000000] - Novice Fly Wing");
					else
						buf += std::string("[^FF0000Deactivate^000000] - Novice Fly Wing");
			}
			if(index == 7){
				if (pc_search_inventory(sd, 23288) >= 0)
					if(!sd->aa.teleport.use_cflywing)
						buf += std::string("[^00FF00Activate^000000] - Compressed Fly Wing");
					else
						buf += std::string("[^FF0000Deactivate^000000] - Compressed Fly Wing");
			}
			if(index == 8){
				if (pc_search_inventory(sd, 23338) >= 0)
					if(!sd->aa.teleport.use_c2flywing)
						buf += std::string("[^00FF00Activate^000000] - Compressed Fly Wing");
					else
						buf += std::string("[^FF0000Deactivate^000000] - Compressed Fly Wing");
			}
			if(index == 9){
				if(sd->aa.teleport.min_hp)
					buf += std::string("Auto Teleport when HP below ^D2691E") + std::to_string(sd->aa.teleport.min_hp) + std::string("^000000% \n");
				if(sd->aa.teleport.delay_nomobmeet)
					buf += std::string("Teleport if no monsters have attacked within ^D2691E") + std::to_string(sd->aa.teleport.delay_nomobmeet) + std::string("^000000s \n");
			}
			script_pushstrcopy(st, buf.c_str());
			break;
		...
		case 8:	// Monsters selection
			if(index == 0){
				if(sd->aa.mobs.aggressive_behavior)
					buf += std::string("^ff0000Avoid^000000 aggressive monsters that are not included in the specified list. \n");
				else
					buf += std::string("^00ff00Engage^000000 aggressive monsters that are not included in the specified list. \n");
				...
				if(sd->aa.mobs.id.size() > 0){
					...
					buf += std::string("No monsters are specified on the list therefore, every monster you encounter will be attacked. \n");
			}
			script_pushstrcopy(st, buf.c_str());
			break;
		...
		case 9:	// Items selection
			if(index == 0){
				switch(sd->aa.prio_item_config){
					case 0:
						buf += std::string("Priority ^ff0000Combat^000000 \n");
						break;
					case 1:
						buf += std::string("Priority ^00ff00Loot^000000 \n");
						break;
				}
				switch(sd->aa.pickup_item_config){
					case 0:
						buf += std::string("Automatically pick up all items on the ground to ensure nothing is missed. \n");
						break;
					case 1:
						buf += std::string("Pick up only the items that are listed and specified in your inventory preferences. \n");
						break;
					case 2:
						buf += std::string("Do not collect any items from the ground ignore all items encountered. \n");
						break;	
				}
				if(sd->aa.pickup_item_id.size() > 0){
					...
					buf += std::string("\nThere are no items specified on the list for collection nothing to pick up. \n");
			}
			script_pushstrcopy(st, buf.c_str());
			break;
		...
		case 10:
			if(index == 0){
				if(!sd->aa.autoslavemode)
					buf += std::string("[^FF0000Disable^000000] - Support Slave");
				else
					buf += std::string("[^00FF00Enable^000000] - Support Slave");
			}
			script_pushstrcopy(st, buf.c_str());
			break;
		...
		case 11:
			if(index == 0){
				if(!sd->aa.autoimpo)
					buf += std::string("[^FF0000OFF^000000] - Impositio Manus");
				else
					buf += std::string("[^00FF00ON^000000] - Impositio Manus");
			}
			script_pushstrcopy(st, buf.c_str());
			break;
		...
		case 12:
			if(index == 0){
				if(!sd->aa.autohealing)
					buf += std::string("[^FF0000OFF^000000] - Heal");
				else
					buf += std::string("[^00FF00ON^000000] - Heal");
			}
			script_pushstrcopy(st, buf.c_str());
			break;
		...
		case 13:
			if(index == 0){
				if(!sd->aa.autoagi)
					buf += std::string("[^FF0000OFF^000000] - Increase Agility");
				else
					buf += std::string("[^00FF00ON^000000] - Increase Agility");
			}
			script_pushstrcopy(st, buf.c_str());
			break;
		...
		case 14:
			if(index == 0){
				if(!sd->aa.autobless)
					buf += std::string("[^FF0000OFF^000000] - Blessing");
				else
					buf += std::string("[^00FF00ON^000000] - Blessing");
			}
			script_pushstrcopy(st, buf.c_str());
			break;
		...
		case 15:
			if(index == 0){
				if(!sd->aa.autoangelus)
					buf += std::string("[^FF0000OFF^000000] - Angelus");
				else
					buf += std::string("[^00FF00ON^000000] - Angelus");
			}
			script_pushstrcopy(st, buf.c_str());
			break;
		...
		case 16:
			if(index == 0){
				if(!sd->aa.autokyrie)
					buf += std::string("[^FF0000OFF^000000] - Kyrie Eleison");
				else
					buf += std::string("[^00FF00ON^000000] - Kyrie Eleison");
			}
			script_pushstrcopy(st, buf.c_str());
			break;
		...
		case 17:
			if(index == 0){
				if(!sd->aa.automagni)
					buf += std::string("[^FF0000OFF^000000] - Magnificat");
				else
					buf += std::string("[^00FF00ON^000000] - Magnificat");
			}
			script_pushstrcopy(st, buf.c_str());
			break;
		...
		case 18:
			if(index == 0){
				if(!sd->aa.autogloria)
					buf += std::string("[^FF0000OFF^000000] - Gloria");
				else
					buf += std::string("[^00FF00ON^000000] - Gloria");
			}
			script_pushstrcopy(st, buf.c_str());
			break;
		...
		case 19:
			if(index == 0){
				if(!sd->aa.autosuffra)
					buf += std::string("[^FF0000OFF^000000] - Suffragium");
				else
					buf += std::string("[^00FF00ON^000000] - Suffragium");
			}
			script_pushstrcopy(st, buf.c_str());
			break;
		...
		case 20:
			if(index == 0){
				if(!sd->aa.autoassump)
					buf += std::string("[^FF0000OFF^000000] - Assumptio");
				else
					buf += std::string("[^00FF00ON^000000] - Assumptio");
			}
			script_pushstrcopy(st, buf.c_str());
			break;
		...
		case 21:
			if(index == 0){
				if(!sd->aa.autoresu)
					buf += std::string("[^FF0000OFF^000000] - Resurrection");
				else
					buf += std::string("[^00FF00ON^000000] - Resurrection");
			}
			script_pushstrcopy(st, buf.c_str());
			break;
		...
		case 22:
			if(index == 0){
				if(!sd->aa.tokenofsieg)
					buf += std::string("[^FF0000OFF^000000] - Token Of Siegfried");
				else
					buf += std::string("[^00FF00ON^000000] - Token Of Siegfried");
			}
			script_pushstrcopy(st, buf.c_str());
			break;
		...
		case 23:
			if(index == 0){
				if(sd->aa.manualwalk)
					buf += std::string("[^FF0000Disable^000000] - Auto Movement");
				else
					buf += std::string("[^00FF00Enable^000000] - Auto Movement");
			}
			script_pushstrcopy(st, buf.c_str());
			break;
	}
...
BUILDIN_FUNC(getautoattackint)
{
	...
	switch(id){
		...
		case 7: // Teleport
			switch(index){
				...
				case 1:
					...
					break;
				case 2:
					num = sd->aa.teleport.use_l1flywing;
					break;
				case 3:
					num = sd->aa.teleport.use_gflywing;
					break;
				case 4:
					num = sd->aa.teleport.use_nflywing;
					break;
				case 5:
					num = sd->aa.teleport.use_iflywing;
					break;
				case 6:
					num = sd->aa.teleport.use_n2flywing;
					break;
				case 7:
					num = sd->aa.teleport.use_cflywing;
					break;
				case 8:
					num = sd->aa.teleport.use_c2flywing;
					break;
				case 9:
		...
		case 18:
			if(index == 0)
				num = sd->aa.autogloria;

			script_pushint(st, num);
			break;
		case 19:
			if(index == 0)
				num = sd->aa.autosuffra;

			script_pushint(st, num);
			break;
		case 20:
			if(index == 0)
				num = sd->aa.autoassump;
		...
		case 22:
			if(index == 0)
				num = sd->aa.tokenofsieg;

			script_pushint(st, num);
			break;
		case 23: 
			if(index == 0)
				num = sd->aa.manualwalk;
...
BUILDIN_FUNC(setautoattack)
{
	...
	if(result.size() > 0){
		...
		switch(id){
				...
				if(result.size() == 3){ // id = 7 - teleport (index,value)
					...
					switch(index){
						...
						case 1:
							...
							break;
						case 2:
							sd->aa.teleport.use_l1flywing = std::stoi(result.at(2));
							break;
						case 3:
							sd->aa.teleport.use_gflywing = std::stoi(result.at(2));
							break;
						case 4:
							sd->aa.teleport.use_nflywing = std::stoi(result.at(2));
							break;
						case 5:
							sd->aa.teleport.use_iflywing = std::stoi(result.at(2));
							break;
						case 6:
							sd->aa.teleport.use_n2flywing = std::stoi(result.at(2));
							break;
						case 7:
							sd->aa.teleport.use_cflywing = std::stoi(result.at(2));
							break;
						case 8:
							sd->aa.teleport.use_c2flywing = std::stoi(result.at(2));
							break;
						case 9:
			...
			case 10:
				if(result.size() == 2) 
					sd->aa.autoslavemode = std::stoi(result.at(1));
				break;
			case 11:
				if(result.size() == 2)
					sd->aa.autoimpo = std::stoi(result.at(1));
				break;
			case 12:
				if(result.size() == 2)
					sd->aa.autohealing = std::stoi(result.at(1));
				break;
			case 13:
				if(result.size() == 2)
					sd->aa.autoagi = std::stoi(result.at(1));
				break;
			case 14:
				if(result.size() == 2)
					sd->aa.autobless = std::stoi(result.at(1));
				break;
			case 15:
				if(result.size() == 2)
					sd->aa.autoangelus = std::stoi(result.at(1));
				break;
			case 16:
				if(result.size() == 2)
					sd->aa.autokyrie = std::stoi(result.at(1));
				break;
			case 17:
				if(result.size() == 2)
					sd->aa.automagni = std::stoi(result.at(1));
				break;
			case 18:
				if(result.size() == 2)
					sd->aa.autogloria = std::stoi(result.at(1));
				break;
			case 19:
				if(result.size() == 2)
					sd->aa.autosuffra = std::stoi(result.at(1));
				break;
			case 20:
				if(result.size() == 2)
					sd->aa.autoassump = std::stoi(result.at(1));
				break;
			case 21:
				if(result.size() == 2)
					sd->aa.autoresu = std::stoi(result.at(1));
				break;
			case 22:
				if(result.size() == 2)
					sd->aa.tokenofsieg = std::stoi(result.at(1));
				break;
			case 23:
				if(result.size() == 2) 
					sd->aa.manualwalk = std::stoi(result.at(1));
				break;
			...
			case 24:
				...
				sd->aa.tokenofsieg = 0;
				sd->aa.manualwalk = 0;
				sd->aa.teleport.use_teleport = 1;
				sd->aa.teleport.use_flywing = 1;
				sd->aa.teleport.use_l1flywing = 1;
				sd->aa.teleport.use_gflywing = 1;
				sd->aa.teleport.use_nflywing = 1;
				sd->aa.teleport.use_iflywing = 1;
				sd->aa.teleport.use_n2flywing = 1;
				sd->aa.teleport.use_cflywing = 1;
				sd->aa.teleport.use_c2flywing = 1;
