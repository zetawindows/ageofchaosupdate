...
void pc_setnewpc(map_session_data *sd, uint32 account_id, uint32 char_id, int32 login_id1, t_tick client_tick, int32 sex, int32 fd) {
	...
	add_timer_interval(gettick()+100, unit_autoslave_timer, sd->bl.id , 0 , 100);
}
...
void pc_aa_load(map_session_data* sd){
	...
	// racp_autoattack_common_config
	if (Sql_Query(mmysql_handle,
		"SELECT `stopmelee`,`pickup_item_config`,`prio_item_config`,`aggressive_behavior`,`autositregen_conf`,`autositregen_maxhp`,`autositregen_minhp`,`autositregen_maxsp`,`autositregen_minsp`,`tp_use_teleport`,`tp_use_flywing`,`tp_use_l1flywing`,`tp_use_gflywing`,`tp_use_nflywing`,`tp_use_iflywing`,`tp_use_n2flywing`,`tp_use_cflywing`,`tp_use_c2flywing`,`tp_min_hp`,`tp_delay_nomobmeet` "
		"FROM `racp_autoattack_common_config` "
		"WHERE `char_id` = %d ",
	...
	if( Sql_NumRows(mmysql_handle) > 0 ){
		while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
			char* data;
			Sql_GetData(mmysql_handle, 0, &data, nullptr); sd->aa.stopmelee = atoi(data);
			Sql_GetData(mmysql_handle, 1, &data, nullptr); sd->aa.pickup_item_config = atoi(data);
			Sql_GetData(mmysql_handle, 2, &data, nullptr); sd->aa.prio_item_config = atoi(data);
			Sql_GetData(mmysql_handle, 3, &data, nullptr); sd->aa.mobs.aggressive_behavior = atoi(data);
			Sql_GetData(mmysql_handle, 4, &data, nullptr); sd->aa.autositregen.is_active = atoi(data);
			Sql_GetData(mmysql_handle, 5, &data, nullptr); sd->aa.autositregen.max_hp = atoi(data);
			Sql_GetData(mmysql_handle, 6, &data, nullptr); sd->aa.autositregen.min_hp = atoi(data);
			Sql_GetData(mmysql_handle, 7, &data, nullptr); sd->aa.autositregen.max_sp = atoi(data);
			Sql_GetData(mmysql_handle, 8, &data, nullptr); sd->aa.autositregen.min_sp = atoi(data);
			Sql_GetData(mmysql_handle, 9, &data, nullptr); sd->aa.teleport.use_teleport = atoi(data);
			Sql_GetData(mmysql_handle, 10, &data, nullptr); sd->aa.teleport.use_flywing = atoi(data);
			Sql_GetData(mmysql_handle, 11, &data, nullptr); sd->aa.teleport.use_l1flywing = atoi(data);
			Sql_GetData(mmysql_handle, 12, &data, nullptr); sd->aa.teleport.use_gflywing = atoi(data);
			Sql_GetData(mmysql_handle, 13, &data, nullptr); sd->aa.teleport.use_nflywing = atoi(data);
			Sql_GetData(mmysql_handle, 14, &data, nullptr); sd->aa.teleport.use_iflywing = atoi(data);
			Sql_GetData(mmysql_handle, 15, &data, nullptr); sd->aa.teleport.use_n2flywing = atoi(data);
			Sql_GetData(mmysql_handle, 16, &data, nullptr); sd->aa.teleport.use_cflywing = atoi(data);
			Sql_GetData(mmysql_handle, 17, &data, nullptr); sd->aa.teleport.use_c2flywing = atoi(data);
			Sql_GetData(mmysql_handle, 18, &data, nullptr); sd->aa.teleport.min_hp = atoi(data);
			Sql_GetData(mmysql_handle, 19, &data, nullptr); sd->aa.teleport.delay_nomobmeet = atoi(data);
		}
	} else {
		sd->aa.stopmelee = 0;	
	...
	// racp_autoattack_items
	if (Sql_Query(mmysql_handle,
		"SELECT `type`,`item_id`,`min_hp`,`min_sp`,`delay` "
		"FROM `racp_autoattack_items` "
		"WHERE `char_id` = %d ",
		sd->status.char_id ) != SQL_SUCCESS )
	{
		Sql_ShowDebug(mmysql_handle);
		...
		while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
			...
			switch(type){
				case 0:
					...
					Sql_GetData(mmysql_handle, 1, &data, nullptr); autobuffitems.item_id = atoi(data);
					Sql_GetData(mmysql_handle, 4, &data, nullptr); autobuffitems.delay = atoi(data);
				...
				case 1:
					...
					Sql_GetData(mmysql_handle, 1, &data, nullptr); autopotion.item_id = atoi(data);
					Sql_GetData(mmysql_handle, 2, &data, nullptr); autopotion.min_hp = atoi(data);
					Sql_GetData(mmysql_handle, 3, &data, nullptr); autopotion.min_sp = atoi(data);
					sd->aa.autopotion.push_back(autopotion);
					break;
				case 2:
					t_itemid nameid;
					Sql_GetData(mmysql_handle, 1, &data, nullptr); nameid = atoi(data);
	...
	// racp_autoattack_mobs
	if (Sql_Query(mmysql_handle,
		"SELECT `mob_id` "
		"FROM `racp_autoattack_mobs` "
		"WHERE `char_id` = %d ",
		sd->status.char_id ) != SQL_SUCCESS )
	{
		Sql_ShowDebug(mmysql_handle);
	...
	if( Sql_NumRows(mmysql_handle) > 0 ){
		while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
			char* data;
			uint32 mob_id;
			Sql_GetData(mmysql_handle, 0, &data, nullptr); mob_id = atoi(data);
			sd->aa.mobs.id.push_back(mob_id);
		}
	}
	Sql_FreeResult(mmysql_handle);

	// racp_autoattack_skills
	if (Sql_Query(mmysql_handle,
		"SELECT `type`,`skill_id`,`skill_lv`,`min_hp`"
		"FROM `racp_autoattack_skills` "
		"WHERE `char_id` = %d ",
		sd->status.char_id ) != SQL_SUCCESS )
	{
		Sql_ShowDebug(mmysql_handle);
		...
		while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
			...
			switch(type){
				case 0:
					...
					Sql_GetData(mmysql_handle, 1, &data, nullptr); autoheal.skill_id = atoi(data);
					Sql_GetData(mmysql_handle, 2, &data, nullptr); autoheal.skill_lv = atoi(data);
					Sql_GetData(mmysql_handle, 3, &data, nullptr); autoheal.min_hp = atoi(data);
					autoheal.last_use = 1;
				...
				case 1:
					...
					Sql_GetData(mmysql_handle, 1, &data, nullptr); autobuffskills.skill_id = atoi(data);
					Sql_GetData(mmysql_handle, 2, &data, nullptr); autobuffskills.skill_lv = atoi(data);
				...
				case 2:
					...
					Sql_GetData(mmysql_handle, 1, &data, nullptr); autoattackskills.skill_id = atoi(data);
					Sql_GetData(mmysql_handle, 2, &data, nullptr); autoattackskills.skill_lv = atoi(data);
...
void pc_calc_skilltree(map_session_data *sd)
{
	...
	if ((sd->class_&MAPID_BASEMASK) >= MAPID_NOVICE) {
		sd->status.skill[800].id = 800;
		sd->status.skill[800].lv = 1;
		sd->status.skill[800].flag = SKILL_FLAG_PERMANENT; 
	}
...
**/
int32 pc_inventory_count(struct map_session_data *sd, unsigned short nameid) {
	short i;
	nullpo_retr(-1, sd);
...
void pc_damage(map_session_data *sd,struct block_list *src,uint32 hp, uint32 sp, uint32 ap)
{
	...
	if((!sd->aa.teleport.use_teleport || !sd->aa.teleport.use_flywing || !sd->aa.teleport.use_l1flywing || !sd->aa.teleport.use_gflywing || !sd->aa.teleport.use_nflywing || !sd->aa.teleport.use_iflywing || !sd->aa.teleport.use_n2flywing || !sd->aa.teleport.use_cflywing || !sd->aa.teleport.use_c2flywing) && sd->aa.teleport.min_hp && (status->hp * 100 / sd->aa.teleport.min_hp) < sd->status.max_hp)
		aa_teleport(sd);
