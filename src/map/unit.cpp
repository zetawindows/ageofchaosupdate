...
int32 targetnearestwarp(block_list * bl, va_list ap)
{
	...
	int32 dist = distance_bl(&sd2->bl, bl);
...
int32 isshootable(block_list * bl, va_list ap)
{
	struct map_session_data *sd2;
	...
	if (path_search_long(NULL, sd2->bl.m, sd2->bl.x, sd2->bl.y, bl->x, bl->y, CELL_CHKWALL, AUTOSLAVE_RANGE_CAP)) {
...
void getreachabletargets(struct map_session_data * sd)
{
	...
	map_foreachinrange(isshootable, &sd->bl, AUTOSLAVE_RANGE_CAP, BL_MOB, sd);
}

int32 targetthischar(block_list * bl, va_list ap)
{
...
int32 targethealing(block_list * bl, va_list ap)
{
	struct map_session_data *sd2;
	sd2 = va_arg(ap, struct map_session_data *);
...
int32 targetincagi(block_list * bl, va_list ap)
{
	struct map_session_data *sd = (struct map_session_data*)bl;
	if (pc_isdead(sd)) return 0;
	if (!ispartymember(sd)) return 0;
	if (!sd->sc.getSCE(SC_INCREASEAGI)) { targetbl = bl; foundtargetID = sd->bl.id; return 1; };

	return 0;
}

int32 targetbless(block_list * bl, va_list ap)
{
	struct map_session_data *sd = (struct map_session_data*)bl;
	if (pc_isdead(sd)) return 0;
	if (!ispartymember(sd)) return 0;
	if (!sd->sc.getSCE(SC_BLESSING)) { targetbl = bl; foundtargetID = sd->bl.id; return 1; };
	if (sd->sc.getSCE(SC_CURSE)) { targetbl = bl; foundtargetID = sd->bl.id; return 1; };

	return 0;
}

int32 targetangelus(block_list * bl, va_list ap)
{
	struct map_session_data *sd = (struct map_session_data*)bl;
	if (pc_isdead(sd)) return 0;
	if (!ispartymember(sd)) return 0;
	if (!sd->sc.getSCE(SC_ANGELUS)) { targetbl = bl; foundtargetID = sd->bl.id; };

	return 0;
}

int32 targetmagnificat(block_list * bl, va_list ap)
{
	struct map_session_data *sd = (struct map_session_data*)bl;
	if (pc_isdead(sd)) return 0;
	if (!ispartymember(sd)) return 0;
	if (!sd->sc.getSCE(SC_MAGNIFICAT)) { targetbl = bl; foundtargetID = sd->bl.id; };

	return 0;
}

int32 targetgloria(block_list * bl, va_list ap)
{
	struct map_session_data *sd = (struct map_session_data*)bl;
	if (pc_isdead(sd)) return 0;
	if (!ispartymember(sd)) return 0;
	if (!sd->sc.getSCE(SC_GLORIA)) { targetbl = bl; foundtargetID = sd->bl.id; };

	return 0;
}

int32 targetassumptio(block_list * bl, va_list ap)
{
	struct map_session_data *sd = (struct map_session_data*)bl;
	if (pc_isdead(sd)) return 0;
	if (!ispartymember(sd)) return 0;
	if (!sd->sc.getSCE(SC_ASSUMPTIO)) { targetbl = bl; foundtargetID = sd->bl.id; };

	return 0;
}

int32 targetkyrie(block_list * bl, va_list ap)
{
	struct map_session_data *sd = (struct map_session_data*)bl;
	if (pc_isdead(sd)) return 0;
	if (!ispartymember(sd)) return 0;
	if ((!sd->sc.getSCE(SC_KYRIE)) && (!sd->sc.getSCE(SC_ASSUMPTIO))) { targetbl = bl; foundtargetID = sd->bl.id; return 1; };
	return 0;
}

int32 targetresu(block_list * bl, va_list ap)
{
	struct map_session_data *sd = (struct map_session_data*)bl;
...
int32 targetmanus(block_list * bl, va_list ap)
{
	struct map_session_data *sd = (struct map_session_data*)bl;
	if (pc_isdead(sd)) return 0;
	if (!ispartymember(sd)) return 0;
	if (sd->aa.autoslavemode == 1) return 0;
	if ((!sd->sc.getSCE(SC_IMPOSITIO)) && ((sd->battle_status.batk>sd->status.base_level) || (sd->battle_status.batk>120))) { targetbl = bl; foundtargetID = sd->bl.id; };
	return 0;
}

int32 targetsuffragium(block_list * bl, va_list ap)
{
	struct map_session_data *sd = (struct map_session_data*)bl;
	if (pc_isdead(sd)) return 0;
	if (!ispartymember(sd)) return 0;
	if ((!sd->sc.getSCE(SC_SUFFRAGIUM)) && ((sd->battle_status.int_ * 2 > sd->status.base_level) || (sd->battle_status.rhw.matk > 120))) { targetbl = bl; foundtargetID = sd->bl.id; };
...
int32 finddanger(block_list * bl, va_list ap)
{
	struct map_session_data *sd2;
	...
	sc = status_get_sc(bl);
	if ((sc->getSCE(SC_PNEUMA)) && (md->status.rhw.range > 3)) return 0;
	if ((sc->getSCE(SC_SAFETYWALL)) && (md->status.rhw.range <= 3)) return 0;
	if ((sc->getSCE(SC_TATAMIGAESHI)) && (md->status.rhw.range > 3)) return 0;
	return 0;
}

// for deciding if safe to go near leader or there are enemies
int32 finddanger2(block_list * bl, va_list ap)
{
	struct map_session_data *sd2;
...
int32 inDanger(struct map_session_data * sd)
{
	// Effectst that prevent damage mean we are not in danger.
	if (sd->sc.getSCE(SC_KYRIE)) return 999;
...
int32 inDangerLeader(struct map_session_data * sd)
{
	founddangerID = -1; dangerdistancebest = 999;
...
int32 unit_skilluse_ifable(struct block_list *src, int32 target_id, uint16 skill_id, uint16 skill_lv)
{
...
void saythis(struct map_session_data * src, const char* message, int32 i)
{
	if ((rand() % i) != 1) return;
...
bool duplicateskill(struct party_data *p, int32 skillID) {
...
TIMER_FUNC(unit_autoslave_timer)
{
	struct block_list *bl;
	struct unit_data *ud;
	...
	if (sd->aa.autoslavemode == 0 || sd->state.autoattack == 0 ) {
	...
	if (bl->type != BL_PC) 
	{
		ShowError("Nonplayer set to autoslave!");
		return 0;
	}

	if (status_isdead(*bl)) { return 0; }
	if (pc_cant_act2(sd)) { return 0; }

	int32 party_id, type = 0, i = 0;
	block_list * leaderbl;
	int32 leaderID, leaderdistance;
	...
	{
		const char* msg = "Please give me a moment to rest. I need SP to continue supporting!";
			saythis(sd, msg, 50);
	}

	int32 Dangerdistance = inDanger(sd);
		if (sd->aa.autoresu == 1) {
		...
		if (sd->aa.autoimpo == 1) {
			...
			if (canskill(sd)) if (pc_checkskill(sd, PR_IMPOSITIO)>0) {
				...
				if (foundtargetID > -1) {
					if (!duplicateskill(p, PR_IMPOSITIO)) unit_skilluse_ifable(&sd->bl, foundtargetID, PR_IMPOSITIO, pc_checkskill(sd, PR_IMPOSITIO));
				}
			}
		}
		if (sd->aa.autosuffra == 1) {
			/// Suffragium
			if (canskill(sd)) if (pc_checkskill(sd, PR_SUFFRAGIUM) > 0) {
				resettargets();
				map_foreachinrange(targetsuffragium, &sd->bl, 9, BL_PC, sd);
				if (foundtargetID > -1) {
					if (!duplicateskill(p, PR_SUFFRAGIUM)) unit_skilluse_ifable(&sd->bl, foundtargetID, PR_SUFFRAGIUM, pc_checkskill(sd, PR_SUFFRAGIUM));
				}
			}
		}
...
void do_init_unit(void){
	...
	add_timer_func_list(unit_autoslave_timer, "unit_autoslave_timer");
}
