...
void chrif_aa_save(map_session_data* sd){
	...
	if( SQL_ERROR == Sql_Query( mmysql_handle, "INSERT INTO `racp_autoattack_common_config` (`char_id`,`stopmelee`,`pickup_item_config`,`prio_item_config`,`aggressive_behavior`,`autositregen_conf`,`autositregen_maxhp`,`autositregen_minhp`,`autositregen_maxsp`,`autositregen_minsp`,`tp_use_teleport`,`tp_use_flywing`,`tp_use_l1flywing`,`tp_use_gflywing`,`tp_use_nflywing`,`tp_use_iflywing`,`tp_use_n2flywing`,`tp_use_cflywing`,`tp_use_c2flywing`,`tp_min_hp`,`tp_delay_nomobmeet`) VALUES (%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d) ON DUPLICATE KEY UPDATE `stopmelee` = %d, `pickup_item_config` = %d, `prio_item_config` = %d, `aggressive_behavior` = %d, `autositregen_conf` = %d, `autositregen_maxhp` = %d, `autositregen_minhp` = %d, `autositregen_maxsp` = %d, `autositregen_minsp` = %d, `tp_use_teleport` = %d, `tp_use_flywing` = %d, `tp_use_l1flywing` = %d, `tp_use_gflywing` = %d, `tp_use_nflywing` = %d, `tp_use_iflywing` = %d, `tp_use_n2flywing` = %d, `tp_use_cflywing` = %d, `tp_use_c2flywing` = %d, `tp_min_hp` = %d, `tp_delay_nomobmeet` = %d ", sd->status.char_id, sd->aa.stopmelee, sd->aa.pickup_item_config, sd->aa.prio_item_config, sd->aa.mobs.aggressive_behavior, sd->aa.autositregen.is_active, sd->aa.autositregen.max_hp, sd->aa.autositregen.min_hp, sd->aa.autositregen.max_sp, sd->aa.autositregen.min_sp, sd->aa.teleport.use_teleport, sd->aa.teleport.use_flywing, sd->aa.teleport.use_l1flywing, sd->aa.teleport.use_gflywing, sd->aa.teleport.use_nflywing, sd->aa.teleport.use_iflywing, sd->aa.teleport.use_n2flywing, sd->aa.teleport.use_cflywing, sd->aa.teleport.use_c2flywing, sd->aa.teleport.min_hp, sd->aa.teleport.delay_nomobmeet, sd->aa.stopmelee, sd->aa.pickup_item_config, sd->aa.prio_item_config, sd->aa.mobs.aggressive_behavior, sd->aa.autositregen.is_active, sd->aa.autositregen.max_hp, sd->aa.autositregen.min_hp, sd->aa.autositregen.max_sp, sd->aa.autositregen.min_sp, sd->aa.teleport.use_teleport, sd->aa.teleport.use_flywing, sd->aa.teleport.use_l1flywing, sd->aa.teleport.use_gflywing, sd->aa.teleport.use_nflywing, sd->aa.teleport.use_iflywing, sd->aa.teleport.use_n2flywing, sd->aa.teleport.use_cflywing, sd->aa.teleport.use_c2flywing, sd->aa.teleport.min_hp, sd->aa.teleport.delay_nomobmeet ) ){
		Sql_ShowDebug(mmysql_handle);
	}

	//clean racp_autoattack_items
	if( SQL_ERROR == Sql_Query( mmysql_handle, "DELETE FROM `racp_autoattack_items` WHERE `char_id` = %d", sd->status.char_id ) ){
		Sql_ShowDebug(mmysql_handle);
	}
	//insert racp_autoattack_items - 0 - autobuffitems
	if(!sd->aa.autobuffitems.empty()){
		for(auto &itAutobuffitem : sd->aa.autobuffitems){
			if( SQL_ERROR == Sql_Query( mmysql_handle, "INSERT INTO `racp_autoattack_items` (`char_id`,`type`,`item_id`,`delay`) VALUES (%d, 0, %d, %d)", sd->status.char_id, itAutobuffitem.item_id, itAutobuffitem.delay ) ){
				Sql_ShowDebug(mmysql_handle);
			}
		}
	}
	//insert racp_autoattack_items - 1 - autopotion
	if(!sd->aa.autopotion.empty()){
		for(auto &itAutopotion : sd->aa.autopotion){
			if( SQL_ERROR == Sql_Query( mmysql_handle, "INSERT INTO `racp_autoattack_items` (`char_id`,`type`,`item_id`,`min_hp`,`min_sp`) VALUES (%d, 1, %d, %d, %d)", sd->status.char_id, itAutopotion.item_id, itAutopotion.min_hp, itAutopotion.min_sp ) ){
				Sql_ShowDebug(mmysql_handle);
			}
		}
	}
	//insert racp_autoattack_items - 2 - pickup_item_id
	if(!sd->aa.pickup_item_id.empty()){
		for (i=0; i<sd->aa.pickup_item_id.size(); i++){
			if( SQL_ERROR == Sql_Query( mmysql_handle, "INSERT INTO `racp_autoattack_items` (`char_id`,`type`,`item_id`) VALUES (%d, 2, %d)", sd->status.char_id, sd->aa.pickup_item_id.at(i) ) ){
				Sql_ShowDebug(mmysql_handle);
			}
		}
	}

	//clean racp_autoattack_mobs
	if( SQL_ERROR == Sql_Query( mmysql_handle, "DELETE FROM `racp_autoattack_mobs` WHERE `char_id` = %d", sd->status.char_id ) ){
		Sql_ShowDebug(mmysql_handle);
	}
	//insert racp_autoattack_mobs
	if(!sd->aa.mobs.id.empty()){
		for (i=0; i<sd->aa.mobs.id.size(); i++){
			if( SQL_ERROR == Sql_Query( mmysql_handle, "INSERT INTO `racp_autoattack_mobs` (`char_id`,`mob_id`) VALUES (%d, %d)", sd->status.char_id, sd->aa.mobs.id.at(i) ) ){
				Sql_ShowDebug(mmysql_handle);
			}
		}
	}

	//clean racp_autoattack_skills
	if( SQL_ERROR == Sql_Query( mmysql_handle, "DELETE FROM `racp_autoattack_skills` WHERE `char_id` = %d", sd->status.char_id ) ){
		Sql_ShowDebug(mmysql_handle);
	}
	//insert racp_autoattack_skills - 0 - autoheal
	if(!sd->aa.autoheal.empty()){
		for(auto &itAutoheal : sd->aa.autoheal){
			if( SQL_ERROR == Sql_Query( mmysql_handle, "INSERT INTO `racp_autoattack_skills` (`char_id`,`type`,`skill_id`,`skill_lv`,`min_hp`) VALUES (%d, 0, %d, %d, %d)", sd->status.char_id, itAutoheal.skill_id, itAutoheal.skill_lv, itAutoheal.min_hp ) ){
				Sql_ShowDebug(mmysql_handle);
			}
		}
	}
	//insert racp_autoattack_skills - 1 - autobuffskills
	if(!sd->aa.autobuffskills.empty()){
		for(auto &itAutobuffskills : sd->aa.autobuffskills){
			if( SQL_ERROR == Sql_Query( mmysql_handle, "INSERT INTO `racp_autoattack_skills` (`char_id`,`type`,`skill_id`,`skill_lv`) VALUES (%d, 1, %d, %d)", sd->status.char_id, itAutobuffskills.skill_id, itAutobuffskills.skill_lv ) ){
				Sql_ShowDebug(mmysql_handle);
			}
		}
	}
	//insert racp_autoattack_skills - 2 - autoattackskills
	if(!sd->aa.autoattackskills.empty()){
		for(auto &itAutoattackskills : sd->aa.autoattackskills){
			if( SQL_ERROR == Sql_Query( mmysql_handle, "INSERT INTO `racp_autoattack_skills` (`char_id`,`type`,`skill_id`,`skill_lv`) VALUES (%d, 2, %d, %d)", sd->status.char_id, itAutoattackskills.skill_id, itAutoattackskills.skill_lv ) ){
				Sql_ShowDebug(mmysql_handle);
			}
		}
	}
...
void chrif_parse_ack_vipActive(int32 fd) {
	...
	if ((flag&0x2)) //isgm
		...
		if((flag&0x1)) { //isvip
			...
			sc_start(nullptr, &sd->bl, SC_VIPSTATE, 100, 1, (vip_time-time(nullptr)) * 1000);
